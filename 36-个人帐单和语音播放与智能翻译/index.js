 ...
 */
 window.onload = function(){
	...
	//因为请求到code后会跳转回到本页面,所以要对我们的地址进行抓取判断,判断是否存在code值
		if((/code=(\S*)\&/).test(str)){                    
            ...
            var code=substr[1];                                                 //如果存在的话,就使用正则match抓取下来      
             ...
             $.ajax({                                
                ...
                success:function(res){ 
					//后台经过两次请求分别通过appid和secret结合code获取到用户access_token与openid,再通过access_token和openid获取用户信息json对象返回用户信息对象,详见get_openid.php页面
                    ...
                    if(typeof res === 'string'){                            
                        ...
                        //userInfo=open;       
                        //$('.weui-tabbar').html(userInfo.nickname);
                    }
                 },
	...
	//$('#myBills').click(function(){
		//window.location.href = 'http://www.tanzhouweb.com/wx/myBills.php?img='+userInfo.headimgurl+'Name='+userInfo.nickname;
	//})
